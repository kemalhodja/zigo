...
test.describe("Billing — Havale / EFT", () => {
  ...
  test("student can open havale checkout and create a transfer request", async ({ page, request }) => {
    await demoLogin(page, "student");

    const configResponse = await request.get("/api/billing/bank-transfer");
    const configBody = (await configResponse.json()) as { data?: { configured?: boolean } };
    if (!configBody.data?.configured) {
      test.skip(true, "Banka havale/EFT yapılandırması bu ortamda mevcut değil.");
    }
