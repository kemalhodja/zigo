  ...
  if (data.mode === "signed-out") {
    ...
    return (
      ...
      <StateCard
        ...
        action={<Link className="font-black text-crystal" href="/auth?next=/duels">{m.common.signIn}</Link>}
...
async function getDuelAccess(): Promise<{ mode: "parent" | "preview" | "signed-out" | "student" | "teacher" }> {

  if (!hasSupabaseEnv()) return { mode: "preview" as const };

  return withSupabaseFallback<{ mode: "parent" | "preview" | "signed-out" | "student" | "teacher" }>(async () => {
