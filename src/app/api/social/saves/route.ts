...
export async function POST(request: Request) {
  try {
    ...
    if (!profile) {
      return NextResponse.json({ error: "Kaydetmek için lütfen giriş yapın." }, { status: 401 });
    }
    ...
    const message = error instanceof z.ZodError
      ? "Lütfen kaydetmek için geçerli bir gönderi seçin."
      : error instanceof Error
        ? error.message
        : "Kaydetme işlemi tamamlanamadı. Lütfen tekrar deneyin.";
