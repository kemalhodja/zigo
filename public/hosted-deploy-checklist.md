...
Full walkthrough: `/staging-deploy.md`
...
Run `supabase/zigo-full-migrations.sql` (migrations **001–033**) in the Supabase SQL editor before first deploy.
Confirm `account_deletion_requests` and `export_user_data()` exist after apply.
...
Open `/readiness` or `/setup` and complete the **Manual role QA** panel.

Also run `/manual-qa-checklist.md` and `/launch-checklist.md`.
