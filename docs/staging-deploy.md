...
2. Open **SQL Editor** → paste and run `supabase/zigo-full-migrations.sql` (33 migrations bundled).
3. Confirm tables exist: `user_subscriptions.current_period_end`, `study_moment_cheers`, `account_deletion_requests`.
...
npm run staging:preflight -- --skip-build   # if build already passed in CI
npm run test:live
node scripts/manual-student-journey.mjs     # against staging URL if dev server proxies; or set detectBaseUrl
```
...
- Supabase: restore from backup or re-run migrations on a fresh project.

See also `docs/hosted-deploy-checklist.md` and `docs/launch-checklist.md`.
